...
let receive_credentials self cancellable =
  let receive_credentials_raw =
    foreign "g_unix_connection_receive_credentials" (t_typ @-> Cancellable.t_typ @-> ptr (ptr_opt Error.t_typ) @-> returning (Credentials.t_typ))
  in
...
let receive_fd self cancellable =
  let receive_fd_raw =
    foreign "g_unix_connection_receive_fd" (t_typ @-> Cancellable.t_typ @-> ptr (ptr_opt Error.t_typ) @-> returning (int32_t))
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
...
let send_credentials self cancellable =
  let send_credentials_raw =
    foreign "g_unix_connection_send_credentials" (t_typ @-> Cancellable.t_typ @-> ptr (ptr_opt Error.t_typ) @-> returning (bool))
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
...
let send_fd self fd cancellable =
  let send_fd_raw =
    foreign "g_unix_connection_send_fd" (t_typ @-> int32_t @-> Cancellable.t_typ @-> ptr (ptr_opt Error.t_typ) @-> returning (bool))
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
