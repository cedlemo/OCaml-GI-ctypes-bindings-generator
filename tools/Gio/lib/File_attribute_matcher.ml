...
let enumerate_next =
  foreign "g_file_attribute_matcher_enumerate_next" (ptr t_typ @-> returning (string_opt))
let matches =
...
let to_string =
  foreign "g_file_attribute_matcher_to_string" (ptr t_typ @-> returning (string_opt))
let unref =
