...
let create_from_data data =
  ...
  let value = create_from_data_raw data err_ptr_ptr in
...
let get_info self path lookup_flags =
  ...
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
  let value = lookup_data_raw self path lookup_flags err_ptr_ptr in
  match (!@ err_ptr_ptr) with
  | None -> Ok value
  | Some _ -> let err_ptr = !@ err_ptr_ptr in
    let _ = Gc.finalise (function | Some e -> Error.free e | None -> () ) err_ptr in
...
let load filename =
  ...
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
  let value = load_raw filename err_ptr_ptr in
  match (!@ err_ptr_ptr) with
  | None -> Ok value
  | Some _ -> let err_ptr = !@ err_ptr_ptr in
    let _ = Gc.finalise (function | Some e -> Error.free e | None -> () ) err_ptr in
