...
let create_certificate_handle =
  foreign "g_tls_database_create_certificate_handle" (t_typ @-> Tls_certificate.t_typ @-> returning (string_opt))
let lookup_certificate_for_handle self handle interaction flags cancellable =
  let lookup_certificate_for_handle_raw =
    foreign "g_tls_database_lookup_certificate_for_handle" (t_typ @-> string @-> Tls_interaction.t_typ @-> Tls_database_lookup_flags.t_view @-> Cancellable.t_typ @-> ptr (ptr_opt Error.t_typ) @-> returning (Tls_certificate.t_typ))
  in
...
let lookup_certificate_issuer self certificate interaction flags cancellable =
  let lookup_certificate_issuer_raw =
    foreign "g_tls_database_lookup_certificate_issuer" (t_typ @-> Tls_certificate.t_typ @-> Tls_interaction.t_typ @-> Tls_database_lookup_flags.t_view @-> Cancellable.t_typ @-> ptr (ptr_opt Error.t_typ) @-> returning (Tls_certificate.t_typ))
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
...
let lookup_certificates_issued_by self issuer_raw_dn interaction flags cancellable =
  let lookup_certificates_issued_by_raw =
    foreign "g_tls_database_lookup_certificates_issued_by" (t_typ @-> ptr Byte_array.t_typ @-> Tls_interaction.t_typ @-> Tls_database_lookup_flags.t_view @-> Cancellable.t_typ @-> ptr (ptr_opt Error.t_typ) @-> returning (ptr_opt List.t_typ))
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
