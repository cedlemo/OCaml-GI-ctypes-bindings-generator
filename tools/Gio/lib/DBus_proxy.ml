...
let call_with_unix_fd_list_sync self method_name parameters flags timeout_msec fd_list cancellable =
  ...
  foreign "g_dbus_proxy_get_cached_property" (t_typ @-> string @-> returning (ptr Variant.t_typ))
...
let get_interface_info =
  foreign "g_dbus_proxy_get_interface_info" (t_typ @-> returning (ptr DBus_interface_info.t_typ))
