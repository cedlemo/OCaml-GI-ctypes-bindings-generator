...
let get_device_path =
  foreign "g_unix_mount_point_get_device_path" (ptr t_typ @-> returning (string_opt))
let get_fs_type =
  foreign "g_unix_mount_point_get_fs_type" (ptr t_typ @-> returning (string_opt))
let get_mount_path =
  foreign "g_unix_mount_point_get_mount_path" (ptr t_typ @-> returning (string_opt))
let get_options =
  foreign "g_unix_mount_point_get_options" (ptr t_typ @-> returning (string_opt))
let guess_can_eject =
...
let guess_name =
  foreign "g_unix_mount_point_guess_name" (ptr t_typ @-> returning (string_opt))
