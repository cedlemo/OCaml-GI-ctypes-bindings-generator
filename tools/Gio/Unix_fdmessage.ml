...
let create =
  foreign "g_unix_fd_message_new" (void @-> returning (ptr Socket_control_message.t_typ))
let create_with_fd_list =
  foreign "g_unix_fd_message_new_with_fd_list" (ptr Unix_fdlist.t_typ @-> returning (ptr Socket_control_message.t_typ))
let append_fd self fd =
  ...
  | Some _ -> let err_ptr = !@ err_ptr_ptr in
    ...
    Error (err_ptr)
let get_fd_list =
  foreign "g_unix_fd_message_get_fd_list" (t_typ @-> returning (ptr Unix_fdlist.t_typ))
