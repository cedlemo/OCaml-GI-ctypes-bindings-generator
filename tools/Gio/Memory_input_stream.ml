...
let create =
  foreign "g_memory_input_stream_new" (void @-> returning (ptr Input_stream.t_typ))
let create_from_bytes =
  foreign "g_memory_input_stream_new_from_bytes" (ptr Bytes.t_typ @-> returning (ptr Input_stream.t_typ))
