...
let create =
  foreign "g_socket_listener_new" (void @-> returning (ptr t_typ))
let accept self cancellable =
  let accept_raw =
    foreign "g_socket_listener_accept" (t_typ @-> ptr_opt Cancellable.t_typ @-> ptr (ptr_opt Object.t_typ) @-> ptr (ptr_opt Error.t_typ) @-> returning (ptr_opt Socket_connection.t_typ))
  in
...
let accept_socket self cancellable =
  let accept_socket_raw =
    foreign "g_socket_listener_accept_socket" (t_typ @-> ptr_opt Cancellable.t_typ @-> ptr (ptr_opt Object.t_typ) @-> ptr (ptr_opt Error.t_typ) @-> returning (ptr_opt Socket.t_typ))
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
  let source_object_ptr = allocate (ptr_opt Object.t_typ) None in
...
let add_address self address _type protocol source_object =
  let add_address_raw =
    foreign "g_socket_listener_add_address" (t_typ @-> ptr Socket_address.t_typ @-> Socket_type.t_view @-> Socket_protocol.t_view @-> ptr_opt Object.t_typ @-> ptr (ptr Socket_address.t_typ) @-> ptr (ptr_opt Error.t_typ) @-> returning (bool))
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
  ...
  let add_any_inet_port_raw =
    foreign "g_socket_listener_add_any_inet_port" (t_typ @-> ptr_opt Object.t_typ @-> ptr (ptr_opt Error.t_typ) @-> returning (uint16_t))
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
...
let add_inet_port self port source_object =
  let add_inet_port_raw =
    foreign "g_socket_listener_add_inet_port" (t_typ @-> uint16_t @-> ptr_opt Object.t_typ @-> ptr (ptr_opt Error.t_typ) @-> returning (bool))
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
...
let add_socket self socket source_object =
  let add_socket_raw =
    foreign "g_socket_listener_add_socket" (t_typ @-> ptr Socket.t_typ @-> ptr_opt Object.t_typ @-> ptr (ptr_opt Error.t_typ) @-> returning (bool))
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
