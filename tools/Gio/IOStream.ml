...
let close self cancellable =
  let close_raw =
    foreign "g_io_stream_close" (t_typ @-> ptr_opt Cancellable.t_typ @-> ptr (ptr_opt Error.t_typ) @-> returning (bool))
  in
...
let get_input_stream =
  foreign "g_io_stream_get_input_stream" (t_typ @-> returning (ptr Input_stream.t_typ))
let get_output_stream =
  foreign "g_io_stream_get_output_stream" (t_typ @-> returning (ptr Output_stream.t_typ))
let has_pending =
