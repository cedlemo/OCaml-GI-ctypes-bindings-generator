...
let create =
  foreign "g_simple_action_new" (string @-> ptr_opt Variant_type.t_typ @-> returning (ptr t_typ))
let create_stateful =
  foreign "g_simple_action_new_stateful" (string @-> ptr_opt Variant_type.t_typ @-> ptr Variant.t_typ @-> returning (ptr t_typ))
let set_enabled =
