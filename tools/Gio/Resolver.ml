...
let get_default =
  foreign "g_resolver_get_default" (void @-> returning (ptr t_typ))
let lookup_by_address self address cancellable =
  let lookup_by_address_raw =
    foreign "g_resolver_lookup_by_address" (t_typ @-> ptr Inet_address.t_typ @-> ptr_opt Cancellable.t_typ @-> ptr (ptr_opt Error.t_typ) @-> returning (string_opt))
  in
...
let lookup_by_name self hostname cancellable =
  let lookup_by_name_raw =
    foreign "g_resolver_lookup_by_name" (t_typ @-> string @-> ptr_opt Cancellable.t_typ @-> ptr (ptr_opt Error.t_typ) @-> returning (ptr_opt List.t_typ))
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
...
let lookup_records self rrname record_type cancellable =
  let lookup_records_raw =
    foreign "g_resolver_lookup_records" (t_typ @-> string @-> Resolver_record_type.t_view @-> ptr_opt Cancellable.t_typ @-> ptr (ptr_opt Error.t_typ) @-> returning (ptr_opt List.t_typ))
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
...
let lookup_service self service protocol domain cancellable =
  let lookup_service_raw =
    foreign "g_resolver_lookup_service" (t_typ @-> string @-> string @-> string @-> ptr_opt Cancellable.t_typ @-> ptr (ptr_opt Error.t_typ) @-> returning (ptr_opt List.t_typ))
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
