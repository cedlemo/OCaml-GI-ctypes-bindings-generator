...
let get_next self =
  let get_next_raw =
    foreign "g_menu_link_iter_get_next" (t_typ @-> ptr (string) @-> ptr (ptr Menu_model.t_typ) @-> returning (bool))
  in
  ...
  (ret, out_link, value)
let get_value =
  foreign "g_menu_link_iter_get_value" (t_typ @-> returning (ptr Menu_model.t_typ))
let next =
