...
let create =
  foreign "g_buffered_input_stream_new" (ptr Input_stream.t_typ @-> returning (ptr Input_stream.t_typ))
let create_sized =
  foreign "g_buffered_input_stream_new_sized" (ptr Input_stream.t_typ @-> uint64_t @-> returning (ptr Input_stream.t_typ))
let fill self count cancellable =
  let fill_raw =
    foreign "g_buffered_input_stream_fill" (t_typ @-> int64_t @-> ptr_opt Cancellable.t_typ @-> ptr (ptr_opt Error.t_typ) @-> returning (int64_t))
  in
...
let read_byte self cancellable =
  let read_byte_raw =
    foreign "g_buffered_input_stream_read_byte" (t_typ @-> ptr_opt Cancellable.t_typ @-> ptr (ptr_opt Error.t_typ) @-> returning (int32_t))
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
