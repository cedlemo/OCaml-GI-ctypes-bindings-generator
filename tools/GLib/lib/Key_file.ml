...
let get_boolean self group_name key =
  ...
  let value = get_boolean_raw self group_name key err_ptr_ptr in
...
let get_comment self group_name key =
  ...
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
  let value = get_comment_raw self group_name key err_ptr_ptr in
  match (!@ err_ptr_ptr) with
  | None -> Ok value
  | Some _ -> let err_ptr = !@ err_ptr_ptr in
    let _ = Gc.finalise (function | Some e -> Error.free e | None -> () ) err_ptr in
...
let get_double self group_name key =
  ...
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
  let value = get_double_raw self group_name key err_ptr_ptr in
  match (!@ err_ptr_ptr) with
  | None -> Ok value
  | Some _ -> let err_ptr = !@ err_ptr_ptr in
    let _ = Gc.finalise (function | Some e -> Error.free e | None -> () ) err_ptr in
...
let get_int64 self group_name key =
  ...
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
  let value = get_int64_raw self group_name key err_ptr_ptr in
  match (!@ err_ptr_ptr) with
  | None -> Ok value
  | Some _ -> let err_ptr = !@ err_ptr_ptr in
    let _ = Gc.finalise (function | Some e -> Error.free e | None -> () ) err_ptr in
...
let get_integer self group_name key =
  ...
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
  let value = get_integer_raw self group_name key err_ptr_ptr in
  match (!@ err_ptr_ptr) with
  | None -> Ok value
  | Some _ -> let err_ptr = !@ err_ptr_ptr in
    let _ = Gc.finalise (function | Some e -> Error.free e | None -> () ) err_ptr in
...
let get_locale_string self group_name key locale =
  ...
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
  let value = get_locale_string_raw self group_name key locale err_ptr_ptr in
  match (!@ err_ptr_ptr) with
  | None -> Ok value
  | Some _ -> let err_ptr = !@ err_ptr_ptr in
    let _ = Gc.finalise (function | Some e -> Error.free e | None -> () ) err_ptr in
...
let get_string self group_name key =
  ...
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
  let value = get_string_raw self group_name key err_ptr_ptr in
  match (!@ err_ptr_ptr) with
  | None -> Ok value
  | Some _ -> let err_ptr = !@ err_ptr_ptr in
    let _ = Gc.finalise (function | Some e -> Error.free e | None -> () ) err_ptr in
...
let get_uint64 self group_name key =
  ...
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
  let value = get_uint64_raw self group_name key err_ptr_ptr in
  match (!@ err_ptr_ptr) with
  | None -> Ok value
  | Some _ -> let err_ptr = !@ err_ptr_ptr in
    let _ = Gc.finalise (function | Some e -> Error.free e | None -> () ) err_ptr in
...
let get_value self group_name key =
  ...
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
  let value = get_value_raw self group_name key err_ptr_ptr in
  match (!@ err_ptr_ptr) with
  | None -> Ok value
  | Some _ -> let err_ptr = !@ err_ptr_ptr in
    let _ = Gc.finalise (function | Some e -> Error.free e | None -> () ) err_ptr in
...
let load_from_data self data length flags =
  ...
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
  let value = load_from_data_raw self data length flags err_ptr_ptr in
  match (!@ err_ptr_ptr) with
  | None -> Ok value
  | Some _ -> let err_ptr = !@ err_ptr_ptr in
    let _ = Gc.finalise (function | Some e -> Error.free e | None -> () ) err_ptr in
...
let load_from_file self file flags =
  ...
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
  let value = load_from_file_raw self file flags err_ptr_ptr in
  match (!@ err_ptr_ptr) with
  | None -> Ok value
  | Some _ -> let err_ptr = !@ err_ptr_ptr in
    let _ = Gc.finalise (function | Some e -> Error.free e | None -> () ) err_ptr in
...
let remove_comment self group_name key =
  ...
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
  let value = remove_comment_raw self group_name key err_ptr_ptr in
  match (!@ err_ptr_ptr) with
  | None -> Ok value
  | Some _ -> let err_ptr = !@ err_ptr_ptr in
    let _ = Gc.finalise (function | Some e -> Error.free e | None -> () ) err_ptr in
...
let remove_group self group_name =
  ...
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
  let value = remove_group_raw self group_name err_ptr_ptr in
  match (!@ err_ptr_ptr) with
  | None -> Ok value
  | Some _ -> let err_ptr = !@ err_ptr_ptr in
    let _ = Gc.finalise (function | Some e -> Error.free e | None -> () ) err_ptr in
...
let remove_key self group_name key =
  ...
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
  let value = remove_key_raw self group_name key err_ptr_ptr in
  match (!@ err_ptr_ptr) with
  | None -> Ok value
  | Some _ -> let err_ptr = !@ err_ptr_ptr in
    let _ = Gc.finalise (function | Some e -> Error.free e | None -> () ) err_ptr in
...
let save_to_file self filename =
  ...
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
  let value = save_to_file_raw self filename err_ptr_ptr in
  match (!@ err_ptr_ptr) with
  | None -> Ok value
  | Some _ -> let err_ptr = !@ err_ptr_ptr in
    let _ = Gc.finalise (function | Some e -> Error.free e | None -> () ) err_ptr in
...
let set_comment self group_name key comment =
  ...
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
  let value = set_comment_raw self group_name key comment err_ptr_ptr in
  match (!@ err_ptr_ptr) with
  | None -> Ok value
  | Some _ -> let err_ptr = !@ err_ptr_ptr in
    let _ = Gc.finalise (function | Some e -> Error.free e | None -> () ) err_ptr in
