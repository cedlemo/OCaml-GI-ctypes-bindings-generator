...
let get_locale_for_key =
  foreign "g_key_file_get_locale_for_key" (ptr t_typ @-> string @-> string @-> string_opt @-> returning (string_opt))
...
let load_from_bytes self bytes flags =
  let load_from_bytes_raw =
    foreign "g_key_file_load_from_bytes" (ptr t_typ @-> ptr Bytes.t_typ @-> Key_file_flags.t_list_view @-> ptr (ptr_opt Error.t_typ) @-> returning (bool))
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
  let ret = load_from_bytes_raw self bytes flags err_ptr_ptr in
  match (!@ err_ptr_ptr) with
  | None -> Ok ret
  | Some _ -> let err_ptr = !@ err_ptr_ptr in
    let _ = Gc.finalise (function | Some e -> Error.free e | None -> () ) err_ptr in
