...
let destroy_link =
  foreign "g_hook_destroy_link" (ptr Hook_list.t_typ @-> ptr t_typ @-> returning (void))
let free =
  foreign "g_hook_free" (ptr Hook_list.t_typ @-> ptr t_typ @-> returning (void))
let insert_before =
  foreign "g_hook_insert_before" (ptr Hook_list.t_typ @-> ptr_opt t_typ @-> ptr t_typ @-> returning (void))
let prepend =
  foreign "g_hook_prepend" (ptr Hook_list.t_typ @-> ptr t_typ @-> returning (void))
let unref =
  foreign "g_hook_unref" (ptr Hook_list.t_typ @-> ptr t_typ @-> returning (void))
