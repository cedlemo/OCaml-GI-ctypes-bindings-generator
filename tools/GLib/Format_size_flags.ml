...
type t = Default | Long_format | Iec_units | Bits
type t_list = t list
...
let of_value v =
  ...
  else if v = Unsigned.UInt32.of_int 4 then Bits
...
let to_value = function
  ...
  | Bits -> Unsigned.UInt32.of_int 4

let list_of_value v =
  let open Unsigned.UInt32 in
  let all_flags = [( 0 , Default ); ( 1 , Long_format ); ( 2 , Iec_units ); ( 4 , Bits )]
  in
