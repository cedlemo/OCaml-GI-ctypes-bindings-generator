...
let create =
  foreign "gtk_grid_new" (void @-> returning (ptr Widget.t_typ))
let attach =
  foreign "gtk_grid_attach" (t_typ @-> ptr Widget.t_typ @-> int32_t @-> int32_t @-> int32_t @-> int32_t @-> returning (void))
let attach_next_to =
  foreign "gtk_grid_attach_next_to" (t_typ @-> ptr Widget.t_typ @-> ptr_opt Widget.t_typ @-> Position_type.t_view @-> int32_t @-> int32_t @-> returning (void))
let get_baseline_row =
  foreign "gtk_grid_get_baseline_row" (t_typ @-> returning (int32_t))
let get_child_at =
  foreign "gtk_grid_get_child_at" (t_typ @-> int32_t @-> int32_t @-> returning (ptr_opt Widget.t_typ))
...
let insert_next_to =
  foreign "gtk_grid_insert_next_to" (t_typ @-> ptr Widget.t_typ @-> Position_type.t_view @-> returning (void))
let insert_row =
