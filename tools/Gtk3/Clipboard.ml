...
let get =
  foreign "gtk_clipboard_get" (ptr Atom.t_typ @-> returning (ptr t_typ))
let get_default =
  foreign "gtk_clipboard_get_default" (ptr Display.t_typ @-> returning (ptr t_typ))
let get_for_display =
  foreign "gtk_clipboard_get_for_display" (ptr Display.t_typ @-> ptr Atom.t_typ @-> returning (ptr t_typ))
let clear =
  foreign "gtk_clipboard_clear" (t_typ @-> returning (void))
let get_display =
  foreign "gtk_clipboard_get_display" (t_typ @-> returning (ptr Display.t_typ))
let get_owner =
  foreign "gtk_clipboard_get_owner" (t_typ @-> returning (ptr_opt Object.t_typ))
...
let set_image =
  foreign "gtk_clipboard_set_image" (t_typ @-> ptr Pixbuf.t_typ @-> returning (void))
let set_text =
...
let wait_for_image =
  foreign "gtk_clipboard_wait_for_image" (t_typ @-> returning (ptr_opt Pixbuf.t_typ))
...
let wait_is_rich_text_available =
  foreign "gtk_clipboard_wait_is_rich_text_available" (t_typ @-> ptr Text_buffer.t_typ @-> returning (bool))
