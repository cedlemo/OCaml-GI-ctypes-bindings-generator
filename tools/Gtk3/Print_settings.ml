...
let create =
  foreign "gtk_print_settings_new" (void @-> returning (ptr t_typ))
let create_from_file file_name =
  let create_from_file_raw =
    foreign "gtk_print_settings_new_from_file" (string @-> ptr (ptr_opt Error.t_typ) @-> returning (ptr_opt t_typ))
  in
...
let create_from_gvariant =
  foreign "gtk_print_settings_new_from_gvariant" (ptr Variant.t_typ @-> returning (ptr t_typ))
let create_from_key_file key_file group_name =
  let create_from_key_file_raw =
    foreign "gtk_print_settings_new_from_key_file" (ptr Key_file.t_typ @-> string_opt @-> ptr (ptr_opt Error.t_typ) @-> returning (ptr_opt t_typ))
  in
  let err_ptr_ptr = allocate (ptr_opt Error.t_typ) None in
  ...
  | Some _ -> let err_ptr = !@ err_ptr_ptr in
    ...
    Error (err_ptr)
let copy =
  foreign "gtk_print_settings_copy" (t_typ @-> returning (ptr t_typ))
