...
let get_display =
  foreign "gtk_selection_data_get_display" (ptr t_typ @-> returning (Display.t_typ))
let get_format =
...
let get_pixbuf =
  foreign "gtk_selection_data_get_pixbuf" (ptr t_typ @-> returning (Pixbuf.t_typ))
let get_selection =
...
let set_pixbuf =
  foreign "gtk_selection_data_set_pixbuf" (ptr t_typ @-> Pixbuf.t_typ @-> returning (bool))
let set_text =
...
let targets_include_rich_text =
  foreign "gtk_selection_data_targets_include_rich_text" (ptr t_typ @-> Text_buffer.t_typ @-> returning (bool))
