...
let create =
  foreign "gtk_im_multicontext_new" (void @-> returning (ptr IMContext.t_typ))
let append_menuitems =
  foreign "gtk_im_multicontext_append_menuitems" (t_typ @-> ptr Menu_shell.t_typ @-> returning (void))
let get_context_id =
