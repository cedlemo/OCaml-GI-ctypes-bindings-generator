...
let create =
  foreign "gtk_style_new" (void @-> returning (ptr t_typ))
let apply_default_background =
  foreign "gtk_style_apply_default_background" (t_typ @-> ptr Context.t_typ @-> ptr Window.t_typ @-> State_type.t_view @-> int32_t @-> int32_t @-> int32_t @-> int32_t @-> returning (void))
let copy =
  foreign "gtk_style_copy" (t_typ @-> returning (ptr t_typ))
let detach =
...
let render_icon =
  foreign "gtk_style_render_icon" (t_typ @-> ptr Icon_source.t_typ @-> Text_direction.t_view @-> State_type.t_view @-> int32_t @-> ptr_opt Widget.t_typ @-> string_opt @-> returning (ptr Pixbuf.t_typ))
let set_background =
  foreign "gtk_style_set_background" (t_typ @-> ptr Window.t_typ @-> State_type.t_view @-> returning (void))
