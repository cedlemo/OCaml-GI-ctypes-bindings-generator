...
let get_default =
  foreign "gtk_settings_get_default" (void @-> returning (ptr_opt t_typ))
let get_for_screen =
  foreign "gtk_settings_get_for_screen" (ptr Screen.t_typ @-> returning (ptr t_typ))
let install_property =
  foreign "gtk_settings_install_property" (ptr Param_spec.t_typ @-> returning (void))
