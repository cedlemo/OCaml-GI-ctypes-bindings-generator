...
let create =
  foreign "gtk_menu_new" (void @-> returning (ptr Widget.t_typ))
let create_from_model =
  foreign "gtk_menu_new_from_model" (ptr Menu_model.t_typ @-> returning (ptr Widget.t_typ))
let get_for_attach_widget =
  foreign "gtk_menu_get_for_attach_widget" (ptr Widget.t_typ @-> returning (ptr List.t_typ))
let attach =
  foreign "gtk_menu_attach" (t_typ @-> ptr Widget.t_typ @-> uint32_t @-> uint32_t @-> uint32_t @-> uint32_t @-> returning (void))
(*Not implemented gtk_menu_attach_to_widget type callback not implemented*)
let detach =
  foreign "gtk_menu_detach" (t_typ @-> returning (void))
let get_accel_group =
  foreign "gtk_menu_get_accel_group" (t_typ @-> returning (ptr Accel_group.t_typ))
let get_accel_path =
  foreign "gtk_menu_get_accel_path" (t_typ @-> returning (string_opt))
let get_active =
  foreign "gtk_menu_get_active" (t_typ @-> returning (ptr Widget.t_typ))
let get_attach_widget =
  foreign "gtk_menu_get_attach_widget" (t_typ @-> returning (ptr Widget.t_typ))
let get_monitor =
...
let place_on_monitor =
  foreign "gtk_menu_place_on_monitor" (t_typ @-> ptr Monitor.t_typ @-> returning (void))
let popdown =
...
let reorder_child =
  foreign "gtk_menu_reorder_child" (t_typ @-> ptr Widget.t_typ @-> int32_t @-> returning (void))
let reposition =
  foreign "gtk_menu_reposition" (t_typ @-> returning (void))
let set_accel_group =
  foreign "gtk_menu_set_accel_group" (t_typ @-> ptr_opt Accel_group.t_typ @-> returning (void))
let set_accel_path =
...
let set_screen =
  foreign "gtk_menu_set_screen" (t_typ @-> ptr_opt Screen.t_typ @-> returning (void))
