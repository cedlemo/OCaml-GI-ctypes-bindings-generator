...
let create =
  foreign "gtk_radio_action_new" (string @-> string_opt @-> string_opt @-> string_opt @-> int32_t @-> returning (ptr t_typ))
...
let join_group =
  foreign "gtk_radio_action_join_group" (t_typ @-> ptr_opt t_typ @-> returning (void))
