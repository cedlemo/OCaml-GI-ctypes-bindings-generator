...
let create =
  foreign "gtk_toolbar_new" (void @-> returning (ptr Widget.t_typ))
let get_drop_index =
...
let get_item_index =
  foreign "gtk_toolbar_get_item_index" (t_typ @-> ptr Tool_item.t_typ @-> returning (int32_t))
let get_n_items =
  foreign "gtk_toolbar_get_n_items" (t_typ @-> returning (int32_t))
let get_nth_item =
  foreign "gtk_toolbar_get_nth_item" (t_typ @-> int32_t @-> returning (ptr_opt Tool_item.t_typ))
...
let insert =
  foreign "gtk_toolbar_insert" (t_typ @-> ptr Tool_item.t_typ @-> int32_t @-> returning (void))
let set_drop_highlight_item =
  foreign "gtk_toolbar_set_drop_highlight_item" (t_typ @-> ptr_opt Tool_item.t_typ @-> int32_t @-> returning (void))
let set_icon_size =
