...
let create =
  foreign "gtk_font_selection_dialog_new" (string @-> returning (ptr Widget.t_typ))
let get_cancel_button =
  foreign "gtk_font_selection_dialog_get_cancel_button" (t_typ @-> returning (ptr Widget.t_typ))
let get_font_name =
  foreign "gtk_font_selection_dialog_get_font_name" (t_typ @-> returning (string_opt))
let get_font_selection =
  foreign "gtk_font_selection_dialog_get_font_selection" (t_typ @-> returning (ptr Widget.t_typ))
let get_ok_button =
  foreign "gtk_font_selection_dialog_get_ok_button" (t_typ @-> returning (ptr Widget.t_typ))
