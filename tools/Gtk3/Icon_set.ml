...
let render_icon =
  foreign "gtk_icon_set_render_icon" (ptr t_typ @-> Style.t_typ @-> Text_direction.t_view @-> State_type.t_view @-> int32_t @-> Widget.t_typ @-> string_opt @-> returning (Pixbuf.t_typ))
let render_icon_pixbuf =
  foreign "gtk_icon_set_render_icon_pixbuf" (ptr t_typ @-> Style_context.t_typ @-> int32_t @-> returning (Pixbuf.t_typ))
let render_icon_surface =
  foreign "gtk_icon_set_render_icon_surface" (ptr t_typ @-> Style_context.t_typ @-> int32_t @-> int32_t @-> Window.t_typ @-> returning (ptr Surface.t_typ))
let unref =
