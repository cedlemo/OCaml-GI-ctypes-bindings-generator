...
let create =
  foreign "gtk_info_bar_new" (void @-> returning (ptr Widget.t_typ))
let add_action_widget =
  foreign "gtk_info_bar_add_action_widget" (t_typ @-> ptr Widget.t_typ @-> int32_t @-> returning (void))
let add_button =
  foreign "gtk_info_bar_add_button" (t_typ @-> string @-> int32_t @-> returning (ptr Button.t_typ))
let get_action_area =
  foreign "gtk_info_bar_get_action_area" (t_typ @-> returning (ptr Widget.t_typ))
let get_content_area =
  foreign "gtk_info_bar_get_content_area" (t_typ @-> returning (ptr Widget.t_typ))
let get_message_type =
  foreign "gtk_info_bar_get_message_type" (t_typ @-> returning (Message_type.t_view))
let get_revealed =
  foreign "gtk_info_bar_get_revealed" (t_typ @-> returning (bool))
...
let set_revealed =
  foreign "gtk_info_bar_set_revealed" (t_typ @-> bool @-> returning (void))
