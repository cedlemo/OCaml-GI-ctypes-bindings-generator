...
let resolve self props =
  let resolve_raw =
    foreign "gtk_gradient_resolve" (ptr t_typ @-> ptr Style_properties.t_typ @-> ptr (ptr Pattern.t_typ) @-> returning (bool))
  in
...
let resolve_for_context =
  foreign "gtk_gradient_resolve_for_context" (ptr t_typ @-> ptr Style_context.t_typ @-> returning (ptr Pattern.t_typ))
let to_string =
