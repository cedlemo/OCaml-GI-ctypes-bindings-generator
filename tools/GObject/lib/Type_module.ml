...
let set_name =
  foreign "g_type_module_set_name" (ptr t_typ @-> string @-> returning (void))
let unuse =
  foreign "g_type_module_unuse" (ptr t_typ @-> returning (void))
let use =
  foreign "g_type_module_use" (ptr t_typ @-> returning (bool))
