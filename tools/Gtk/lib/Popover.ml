...
let create =
  foreign "gtk_popover_new" (Widget.t_typ @-> returning (Widget.t_typ))
let create_from_model =
  foreign "gtk_popover_new_from_model" (Widget.t_typ @-> Menu_model.t_typ @-> returning (Widget.t_typ))
let bind_model =
  foreign "gtk_popover_bind_model" (t_typ @-> Menu_model.t_typ @-> string_opt @-> returning (void))
let get_constrain_to =
  foreign "gtk_popover_get_constrain_to" (t_typ @-> returning (Popover_constraint.t_view))
let get_default_widget =
  foreign "gtk_popover_get_default_widget" (t_typ @-> returning (Widget.t_typ))
let get_modal =
...
let set_default_widget =
  foreign "gtk_popover_set_default_widget" (t_typ @-> Widget.t_typ @-> returning (void))
let set_modal =
...
let set_relative_to =
  foreign "gtk_popover_set_relative_to" (t_typ @-> Widget.t_typ @-> returning (void))
