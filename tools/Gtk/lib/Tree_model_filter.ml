...
let clear_cache =
  foreign "gtk_tree_model_filter_clear_cache" (ptr t_typ @-> returning (void))
let convert_child_iter_to_iter self child_iter =
  ...
  let convert_child_iter_to_iter_raw =
    foreign "gtk_tree_model_filter_convert_child_iter_to_iter" (ptr t_typ @-> ptr Tree_iter.t_typ @-> ptr (Tree_iter.t_typ) @-> returning bool)
  in
...
let convert_child_path_to_path =
  foreign "gtk_tree_model_filter_convert_child_path_to_path" (ptr t_typ @-> ptr Tree_path.t_typ @-> returning (ptr_opt Tree_path.t_typ))
let convert_iter_to_child_iter self filter_iter =
  ...
  let convert_iter_to_child_iter_raw =
    foreign "gtk_tree_model_filter_convert_iter_to_child_iter" (ptr t_typ @-> ptr Tree_iter.t_typ @-> ptr (Tree_iter.t_typ) @-> returning void)
  in
...
let convert_path_to_child_path =
  foreign "gtk_tree_model_filter_convert_path_to_child_path" (ptr t_typ @-> ptr Tree_path.t_typ @-> returning (ptr_opt Tree_path.t_typ))
(*Not implemented gtk_tree_model_filter_get_model return type interface not handled*)
let refilter =
  foreign "gtk_tree_model_filter_refilter" (ptr t_typ @-> returning (void))
...
let set_visible_column =
  foreign "gtk_tree_model_filter_set_visible_column" (ptr t_typ @-> int32_t @-> returning (void))
