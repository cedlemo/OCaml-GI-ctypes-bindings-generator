...
let add_color_stop =
  foreign "gtk_gradient_add_color_stop" (t_typ @-> double @-> ptr Symbolic_color.t_typ @-> returning (void))
let incr_ref =
  foreign "gtk_gradient_ref" (t_typ @-> returning (ptr t_typ))
let resolve self props =
  let resolved_gradient_ptr = allocate (ptr_opt Pattern.t_typ) None in
  let resolve_raw =
    foreign "gtk_gradient_resolve" (t_typ @-> Style_properties.t_typ @-> ptr (ptr Pattern.t_typ) @-> returning bool)
  in
...
let resolve_for_context =
  foreign "gtk_gradient_resolve_for_context" (t_typ @-> Style_context.t_typ @-> returning (ptr Pattern.t_typ))
let to_string =
  foreign "gtk_gradient_to_string" (t_typ @-> returning (string_opt))
let unref =
  foreign "gtk_gradient_unref" (t_typ @-> returning (void))
