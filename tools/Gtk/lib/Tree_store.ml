...
let insert self parent position =
  ...
  let iter = !@ iter_ptr in
  (iter)let insert_after self parent sibling =
  ...
  let iter = !@ iter_ptr in
  (iter)let insert_before self parent sibling =
  ...
  let iter = !@ iter_ptr in
  (iter)(*Not implemented gtk_tree_store_insert_with_valuesv type C Array type for Types.Array tag not implemented*)
let is_ancestor =
