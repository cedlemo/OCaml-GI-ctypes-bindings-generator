...
let get_alignment self =
  ...
  let get_alignment_raw =
    foreign "gtk_misc_get_alignment" (ptr t_typ @-> ptr (float) @-> ptr (float) @-> returning void)
  in
...
let get_padding self =
  ...
  let get_padding_raw =
    foreign "gtk_misc_get_padding" (ptr t_typ @-> ptr (int32_t) @-> ptr (int32_t) @-> returning void)
  in
  ...
  (xpad, ypad)
let set_alignment =
  foreign "gtk_misc_set_alignment" (ptr t_typ @-> float @-> float @-> returning (void))
let set_padding =
  foreign "gtk_misc_set_padding" (ptr t_typ @-> int32_t @-> int32_t @-> returning (void))
