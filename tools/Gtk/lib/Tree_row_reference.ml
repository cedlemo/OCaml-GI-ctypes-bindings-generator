...
let copy =
  foreign "gtk_tree_row_reference_copy" (t_typ @-> returning (ptr t_typ))
let free =
  foreign "gtk_tree_row_reference_free" (t_typ @-> returning (void))
(*Not implemented gtk_tree_row_reference_get_model return type interface not handled*)
let get_path =
  foreign "gtk_tree_row_reference_get_path" (t_typ @-> returning (ptr_opt Tree_path.t_typ))
let valid =
  foreign "gtk_tree_row_reference_valid" (t_typ @-> returning (bool))
let deleted =
