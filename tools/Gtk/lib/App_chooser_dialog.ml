...
let create_for_content_type =
  foreign "gtk_app_chooser_dialog_new_for_content_type" (Window.t_typ @-> Dialog_flags.t_list_view @-> string @-> returning (Widget.t_typ))
let get_heading =
  foreign "gtk_app_chooser_dialog_get_heading" (t_typ @-> returning (string_opt))
let get_widget =
  foreign "gtk_app_chooser_dialog_get_widget" (t_typ @-> returning (Widget.t_typ))
let set_heading =
